...
import com.example.lovepaw.databinding.ActivitySignUpBinding;
...
public class SignUpActivity extends AppCompatActivity {
    ...
    private void setListeners(){
        ...
        binding.buttonSignUp.setOnClickListener(v -> {
            if (Boolean.TRUE.equals(isValidSignUpDetails())){
                signUp();
            }
        });
    ...
    private void signUp(){
        ...
        user.put(Constants.KEY_IDNUMBER,binding.inputIdnumber.getText().toString());
        user.put(Constants.KEY_PASSWORD,binding.inputPassword.getText().toString());
        user.put(Constants.KEY_EMAIL,binding.inputEmail.getText().toString());
        user.put(Constants.KEY_PHONE,binding.inputPhone.getText().toString());
        user.put(Constants.KEY_ADDRESS,binding.inputAddress.getText().toString());
        user.put(Constants.KEY_IMAGE,encodedImage);
        database.collection(Constants.KEY_COLLECTION_USERS)
                .add(user)
                .addOnSuccessListener(documentReference -> {
                    loading(false);
                    preferenceManager.putBoolean(Constants.KEY_IS_SIGEND_IN,true);
    ...
    private Boolean isValidSignUpDetails() {
        if (encodedImage == null) {
            ...
            showToast("請輸入性別");
            return false;
        } else if (binding.inputIdnumber.getText().toString().trim().isEmpty()) {
            showToast("請輸入身份證字號");
            return false;
